...
data_sql = \
    '''
        ...
        (
            ...
            FROM gdelt.event 
            where EventDate = '{get_date}' AND  ActionGeo_ADM1Code = '{ADM1Code}' AND IsRootEvent=1 AND NumMentions >1 AND Actor1Name != '' AND EventCode != '' AND Actor2Name != '' AND SOURCEURL != ''
        ...
        temp_eg as 
        (
            SELECT GlobalEventID, Actor1Name, EventCode, Actor2Name, AllNames, EventDate from temp_event inner join temp_gkg on temp_event.SOURCEURL = temp_gkg.DocumentIdentifier 
        ),
        temp_mentions as 
        (select GLOBALEVENTID, Confidence from gdelt.mentions where EventDate='{get_date}') 
        
        SELECT GlobalEventID, Actor1Name, EventCode, Actor2Name, AllNames, EventDate from temp_eg inner join temp_mentions on temp_eg.GlobalEventID == temp_mentions.GLOBALEVENTID where temp_mentions.Confidence = 100
